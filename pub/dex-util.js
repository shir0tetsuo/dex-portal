...
// LASTLY, DON'T FORGET TO EDIT THE /PART/ TO COONTAIN READABLE ENGLISH (EDITCTRL)
ident_img_array = [
...
function pushValue(id, origin) {
  ...
  if (id == 'idbox') {
    //document.getElementById('display').src = `/pub/${ident_img_array[origin.value-1]}.gif`;
    document.getElementById('display').src = `/pub/${ident_img_array[origin.value-1]}.svg`;
  }
}
...
function getRandLoc() {
  ...
  window.location.replace(`/view/${xxx}${yyy}`)
}
...
// store/stripe
function loadStorePortal() {
  const storeCollection = document.getElementsByClassName('storeQTY')
  for (s in storeCollection) {
    document.getElementsByClassName('storeQTY')[s].value = 0;
  }
}
...
function returnStoreCheckout() {
  const storeCollection = document.getElementsByClassName('storeQTY')
  var chkout = []
  for (s in storeCollection) {
    var objId = document.getElementsByClassName('storeQTY')[s].id
    var objVal = document.getElementsByClassName('storeQTY')[s].value
    if (objId != undefined && objVal >= 1) {
      const storeItem = new StoreItem(objId,objVal);
      chkout.push(storeItem)
    }
  }
  $.post("/storefront/chkout",{chkout: chkout},function(data){
...
function gotoBar() {
  ...
  window.location.replace(`/view/${ref}`)
}
...
function gotoLong() {
  ...
  window.location.replace(`/view/${digiLat}${digiLon}`)
}
...
async function bakeCookieButton() {
  ...
  if (user_email) {
    ...
    sdi = document.getElementById('spirideximport')
    sdi.href = ''
    sdi.innerHTML = ''
...
function GeoConnect(){
  // get geolocation
  navigator.geolocation.getCurrentPosition(triangulateGeo)
}

function triangulateGeo(position) {
  var lat = parseInt(position.coords.latitude + 90),
  lon = parseInt(position.coords.longitude + 180);
  returnAddress = `/view/${lat}${lon}`
  window.location.href = returnAddress;
  if (window.location.href != returnAddress) {
...
function syncImgIdent(pointer) {
  ...
  document.getElementById('display').src = `/pub/${ident_img_array[pointer-1]}.svg`
}
